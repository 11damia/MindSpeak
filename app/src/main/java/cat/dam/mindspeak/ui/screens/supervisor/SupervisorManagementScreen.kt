...
fun SupervisorManagementScreen(
    ...
    // Fetch users method (you'll need to implement this in FirebaseManager)
    LaunchedEffect(Unit) {
        users = firebaseManager.getUsersByRole("Usuari")
    }
            ...
            AddEditUserDialog(
                ...
                onSave = { newUser ->
                    coroutineScope.launch {
                        firebaseManager.registrarUsuari(
                            email = newUser.email ?: "",
                            contrasenya = "defaultPassword", // Consider secure password generation
                            nom = newUser.nom ?: "",
                            cognom = newUser.cognom ?: "",
                            telefon = newUser.telefon,
                            dataNaixement = null,
...
fun AddEditUserDialog(
    ...
    AlertDialog(
        onDismissRequest = onDismiss,
        title = {
            Text(if (isEditing) "Editar Usuari" else "Afegir Usuari")
        },
        text = {
            Column {
                TextField(
