...
fun EmotionRatingScreen(
    ...
    // Crear un fitxer temporal per la foto
    val tempImageFile = remember {
        File.createTempFile(
            "JPEG_${SimpleDateFormat("yyyyMMdd_HHmmss", Locale.getDefault()).format(Date())}_",
            ".jpg",
    ...
    // URI per la imatge capturada per la càmera
    val tempImageUri = remember {
        FileProvider.getUriForFile(
            context,
            "${context.packageName}.fileprovider",
    ...
    val requestCameraPermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (isGranted) {
            // Si el permís és concedit, llavors podem llançar la càmera
            takePhotoLauncher.launch(tempImageUri)
        } else {
            // Si el permís no és concedit, mostrem un missatge
            Toast.makeText(context, "Es necessita permís de càmera per fer fotos", Toast.LENGTH_LONG).show()
        }
    }
    ...
    val requestStoragePermissionLauncher = rememberLauncherForActivityResult(
        ...
        if (isGranted) {
            // Si el permís és concedit, llavors podem obrir la galeria
            pickImageLauncher.launch("image/*")
        } else {
            // Si el permís no és concedit, mostrem un missatge
            Toast.makeText(context, "Es necessita permís d'emmagatzematge per accedir a les imatges", Toast.LENGTH_LONG).show()
    ...
    Column(
        ...
        // Zona de text pels comentaris emocionals
        OutlinedTextField(
            value = comentariEmocional,
            onValueChange = { comentariEmocional = it },
            label = { Text("Com et sents?") },
        ...
        Card(
            ...
            if (imageUri != null) {
                // Mostrar la imatge seleccionada
                Image(
                    painter = rememberAsyncImagePainter(imageUri),
                    contentDescription = "Imatge seleccionada",
                    modifier = Modifier.fillMaxSize(),
                ...
                // Mostrar les opcions per afegir una imatge
                Column(
                    modifier = Modifier.fillMaxSize(),
                    verticalArrangement = Arrangement.Center,
                    horizontalAlignment = Alignment.CenterHorizontally
                    ...
                    Row(
                        ...
                        Button(
                            onClick = {
                                // Comprovar si tenim permís de càmera
                                when (PackageManager.PERMISSION_GRANTED) {
                                    ContextCompat.checkSelfPermission(
                                        context,
                                        Manifest.permission.CAMERA
                                    ) -> {
                                        // Si tenim permís, llancem la càmera
                                        takePhotoLauncher.launch(tempImageUri)
                                    }
                                    else -> {
                                        // Si no tenim permís, el demanem
                                        requestCameraPermissionLauncher.launch(Manifest.permission.CAMERA)
                                    }
                                }
                            },
                        ...
                        Button(
                            onClick = {
                                // Comprovar si tenim permís de lectura d'emmagatzematge
                                when (PackageManager.PERMISSION_GRANTED) {
                                    ContextCompat.checkSelfPermission(
                                        context,
                                        Manifest.permission.READ_EXTERNAL_STORAGE
                                    ) -> {
                                        // Si tenim permís, obrim la galeria
                                        pickImageLauncher.launch("image/*")
                                    }
                                    else -> {
                                        // Si no tenim permís, el demanem
        ...
        // Mostrar un botó per esborrar la imatge si n'hi ha una seleccionada
        if (imageUri != null) {
            Button(
                onClick = { imageUri = null },
                colors = ButtonDefaults.buttonColors(containerColor = Color.Red.copy(alpha = 0.7f)),
        ...
        Button(
            onClick = {
                if (imageUri != null) {
                    ...
                    scope.launch {
                        try {
                            // Pujar la imatge a Supabase
                            val imageUrl = SupabaseStorageUtil.uploadImage(context, imageUri!!)

                            // Crear un registre d'emoció amb l'URL de la imatge
                            val emotionRecord = EmotionRecord(
                                emotionType = emotionType,
                                rating = rating,
                                date = Date(),
...
fun getEmotionColor(emotionType: String): Color {
    return when (emotionType) {
        "ENFADAT" -> Color(0xFFFF5353)
        "TRIST" -> Color(0xFF2E9DFF)
        "POR" -> Color(0xFF894AB8)
        "FELIÇ" -> Color(0xFF4CAF50)
        "ANSIÓS" -> Color(0xFFD97904)
        else -> Color.Gray
    }
}
...
fun getEmotionImage(emotionType: String): Int {
    return when (emotionType) {
        "ENFADAT" -> R.drawable.enfadado
        "TRIST" -> R.drawable.triste
        "POR" -> R.drawable.miedoso
        "FELIÇ" -> R.drawable.feliz
        "ANSIÓS" -> R.drawable.ansioso
